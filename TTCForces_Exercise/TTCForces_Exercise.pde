...
void setup(){
  ...
  agentPos[3] = new Vec2(114,514);
  agentPos[4] = new Vec2(320,420);
  
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(500,500);
  goalPos[3] = new Vec2(300,400  );
  goalPos[4] = new Vec2(500,10);
...
//goal_force algorithm
  Vec2 goal_force = (goal_vel.minus(agentVel[id])).times(k_goal);;
  acc.add(goal_force);
  
  if(goal_vel.length() < 1) return acc;
  
  for(int i = 0; i < numAgents; i++){
    if(i != id){
      float ttc = computeTTC(agentPos[id], agentVel[id], agentRad, agentPos[i], agentVel[i], agentRad);
      
      Vec2 A_NextPos = agentPos[id].plus(agentVel[id].times(ttc));
      Vec2 B_NextPos = agentPos[i].plus(agentVel[i].times(ttc));
      
      Vec2 nextDir = A_NextPos.minus(B_NextPos).normalized();
      
      acc.add(nextDir.times(k_avoid * (1 / ttc)));
    }
  }
  return acc;
}
