 ...
 * 1. Add a function so that when you press the 'r' key, the obstacles move to
  ...
  *2. Currently edge connections of any length are allowed. Update the connectNeighbors()
     ...
     Ans: because it can reduce the number of lines is generated, and help to find a shorter path
...
int closestNode(Vec2 point){
  ...
  int node = 0;
  float min = point.distanceTo(nodePos[0]);
  for(int i = 1; i < numNodes; i++){
    if(nodePos[i].x == point.x && nodePos[i].y == point.y) { // skip the point itself
      continue;
    }
    float distance = nodePos[i].distanceTo(point);
    if(distance < min){
        node = i;
        min = distance;
    }
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  ...
  if(pointPos.x >= boxTopLeft.x && pointPos.x <= boxTopLeft.x + boxW &&
      pointPos.y >= boxTopLeft.y && pointPos.y <= boxTopLeft.y + boxH){
        return true;
  }else{
    return false;
  }
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      //2 update only allow edges if their length is less than 200 pixels
