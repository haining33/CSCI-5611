...
/*
 TODO: 
    1. Try to understand how this Breadth-first Search (BFS) implementation works.
       ...
       you represent nodes using arraylist's index, and the edges are the element in the arraylist, they store the
       indexes of the next node.
       the purpose of the visited list is to avoid getting in a circle and parent list si to remember the path we have visited
       the children nodes are added to the fringe, in first in first out order.
       we use the parent array to find it, just put it the index of the goal to the parent array we can find its parent,
       we record every parent until we find the start point;
    2. Convert this Breadth-first Search to a Depth-First Search.
       Which version BFS or DFS has a smaller maximum fring size?
       DFS has a smaller fring size
    3. Currently, the code sets up a graph which follows this tree-like structure: https://snipboard.io/6BhxRd.jpg
       ...
       Becasue there are nodes that have the same chile, like node 1 and node 3
       Does Breadth-first Search still find the optimal path?
       
       
       bfs still find the optimal path.
 CHALLENGE:
