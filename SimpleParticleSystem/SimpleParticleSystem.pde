...
void update(float dt){
  ...
  //No. 6,  7
  if(leftPressed && upPressed) obstacleVel = new Vec2(-obstacleSpeed/sqrt(2),-obstacleSpeed/sqrt(2));
  if(rightPressed && upPressed) obstacleVel = new Vec2(obstacleSpeed/sqrt(2),-obstacleSpeed/sqrt(2));
  if(leftPressed && downPressed) obstacleVel = new Vec2(-obstacleSpeed/sqrt(2),obstacleSpeed/sqrt(2));
  if(rightPressed && downPressed) obstacleVel = new Vec2(obstacleSpeed/sqrt(2),obstacleSpeed/sqrt(2));
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    //8
    Vec2 acc = gravity.times(dt); //Gravity
    vel[i].add(acc);
